...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except anmmrp.exceptions.UserError:
        raise
    except anmmrp.exceptions.RedirectWarning:
        raise
...
`anmmrp.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `anmmrp.tools.safe_eval`.
""")
    return d
