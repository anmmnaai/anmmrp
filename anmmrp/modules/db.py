...
def is_initialized(cr):
    ...
    """
    return anmmrp.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    try:
        f = anmmrp.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with anmmrp.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in anmmrp.modules.get_modules():
        mod_path = anmmrp.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = anmmrp.modules.get_manifest(i)
...
def has_trigram(cr):
    ...
    pg_trgm module but any similar function will be picked by anmMRP.
