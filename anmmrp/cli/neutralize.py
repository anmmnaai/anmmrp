...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = anmmrp.tools.config.parser
        ...
        opt = anmmrp.tools.config.parse_config(args)

        dbname = anmmrp.tools.config['db_name']
        if not dbname:
        ...
        try:
            with anmmrp.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = anmmrp.modules.neutralize.get_installed_modules(cursor)
                    queries = anmmrp.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    anmmrp.modules.neutralize.neutralize_database(cursor)
