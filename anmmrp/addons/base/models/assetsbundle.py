...
from anmmrp import release, SUPERUSER_ID, _
from anmmrp.http import request
from anmmrp.tools import (func, misc, transpile_javascript,
    is_anmmrp_module, SourceMapGenerator, profiler,
    apply_inheritance_specs)
from anmmrp.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from anmmrp.tools.misc import file_open, file_path
from anmmrp.tools.pycompat import to_text
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    anmmrp.define('{self.name}.bundle.xml', ['@web/core/registry'], function(require){{
                        'use strict';
        ...
        * `t-extend` deprecated attribute, used by the JavaScript Qweb.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<anmmrp>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In anmMRP you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            self._is_transpiled = bool(is_anmmrp_module(super().content))
