...
import anmmrp
from anmmrp import api, fields, models, _
from anmmrp.exceptions import UserError

_logger = logging.getLogger(__name__)

BASE_VERSION = anmmrp.modules.get_manifest('base')['version']
MAX_FAIL_TIME = timedelta(hours=5)  # chosen with a fair roll of the dice

# custom function to call instead of default PostgreSQL's `pg_notify`
ANMMRP_NOTIFY_FUNCTION = os.getenv('ANMMRP_NOTIFY_FUNCTION', 'pg_notify')
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('ANMMRP_NOTIFY_CRON_CHANGES'):
        ...
        try:
            ...
            with db.cursor() as cron_cr:
                ...
                for job_id in (job['id'] for job in jobs):
                    ...
                    registry = anmmrp.registry(db_name)
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        anmmrp.modules.reset_modules_state(cr.dbname)
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            anmmrp.netsvc.log(_logger, logging.DEBUG, 'cron.object.execute', (self._cr.dbname, self._uid, '*', cron_name, server_action_id), depth=log_depth)
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('ANMMRP_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('ANMMRP_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers

    def _notifydb(self):
        """ Wake up the cron workers
        The ANMMRP_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with anmmrp.sql_db.db_connect('postgres').cursor() as cr:
            query = sql.SQL("SELECT {}('cron_trigger', %s)").format(sql.Identifier(ANMMRP_NOTIFY_FUNCTION))
