# -*- coding: utf-8 -*-
"""
The module :mod:`anmmrp.tests.form` provides an implementation of a client form
...
from anmmrp.fields import Command
from anmmrp.tools.safe_eval import safe_eval
...
class Form:
    ...
    * properly handle defaults & onchanges around x2many fields.
    ...
    of :class:`~anmmrp.fields.Many2one` fields, one can assign a recordset::
    ...
    For :class:`~anmmrp.fields.Many2many` fields, the field itself is a
    :class:`~anmmrp.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~anmmrp.fields.One2many` are reified as :class:`~O2MProxy`.

    Because the :class:`~anmmrp.fields.One2many` only exists through its parent,
    ...
    :param record: empty or singleton recordset. An empty recordset will put
                   ...
                   view's data.
    :type record: anmmrp.models.Model
    ...
    :type view: int | str | anmmrp.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~anmmrp.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~anmmrp.fields.One2many` record.
...
def convert_read_to_form(values, fields):
    ...
    for fname, value in values.items():
        ...
        if field_info['type'] == 'one2many':
            ...
            value = anmmrp.fields.Datetime.to_string(value)
        elif field_info['type'] == 'date' and isinstance(value, date):
            value = anmmrp.fields.Date.to_string(value)
        result[fname] = value
    return result
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return anmmrp.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return anmmrp.fields.Date.to_string(value)
    return value
