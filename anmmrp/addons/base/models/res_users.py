...
from anmmrp import api, fields, models, tools, SUPERUSER_ID, _, Command
from anmmrp.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from anmmrp.exceptions import AccessDenied, AccessError, UserError, ValidationError
from anmmrp.http import request, DEFAULT_LANG
from anmmrp.osv import expression
from anmmrp.service.db import check_super
from anmmrp.tools import is_html_empty, partition, collections, frozendict, lazy_property
...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by anmMRP (updates, module installation, ...)'))
        self.env.registry.clear_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: anmmrp.exceptions.AccessDenied when old password is wrong
        :raise: anmmrp.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this anmMRP portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your anmMRP is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "anmMRP in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.anmmrp.com/documentation/17.0/administration/install/deploy.html#https for details.",
                    source
                )
