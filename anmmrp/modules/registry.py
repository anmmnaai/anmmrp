...
import anmmrp
from anmmrp.modules.db import FunctionStatus
from anmmrp.osv.expression import get_unaccent_wrapper
from .. import SUPERUSER_ID
from anmmrp.sql_db import TestCursor
from anmmrp.tools import (
    config, existing_tables, lazy_classproperty,
    lazy_property, sql, Collector, OrderedSet, SQL,
    format_frame
)
from anmmrp.tools.func import locked
from anmmrp.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('anmmrp.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                anmmrp.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                anmmrp.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        self._assertion_report = anmmrp.tests.result.OdooTestResult()
        ...
        self._db = anmmrp.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = anmmrp.modules.db.has_unaccent(cr)
            self.has_trigram = anmmrp.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = anmmrp.api.Environment(cr, SUPERUSER_ID, context)
