...
r"""\
anmMRP HTTP layer / WSGI application
...
:class:`~anmmrp.http.Controller`: class and its companion the
:func:`~anmmrp.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an anmMRP http request. The anmMRP http request is
...
anmMRP URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.anmmrp.com/documentation/17.0/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through anmMRP via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~anmmrp.service.db.list_dbs`:
    ...
    """
    try:
        dbs = anmmrp.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, anmMRP will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': anmmrp.service.common.dispatch,
        'db': anmmrp.service.db.dispatch,
        'object': anmmrp.service.model.dispatch,
    }
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    warnings.warn('anmmrp.http.send_file is deprecated, please use anmmrp.http.Stream instead.', DeprecationWarning, stacklevel=2)
    return _send_file(
        filepath_or_fp,
...
class Stream:
    ...
    def get_response(self, as_attachment=None, immutable=None, **send_file_kwargs):
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`anmmrp.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~anmmrp.http.Controller` can use the :func:`~anmmrp.http.route`:
    ...
    :func:`~anmmrp.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(anmmrp.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['anmmrp', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~anmmrp.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['anmmrp', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are anmMRP addons.
        """
        # Controllers defined outside of anmmrp addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of anmmrp addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, login=None, password=None):
        ...
        with registry.cursor() as cr:
            env = anmmrp.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            ...
            request.env = anmmrp.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~anmmrp.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> anmmrp_ip = socket.gethostbyname('anmmrp.com')
        >>> GeoIP(anmmrp_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~anmmrp.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~anmmrp.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~anmmrp.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            self.env = anmmrp.api.Environment(cr, self.session.uid, self.session.context)
            threading.current_thread().uid = self.env.uid
            try:
                return service_model.retrying(self._serve_ir_http, self.env)
            except Exception as exc:
                if isinstance(exc, HTTPException) and exc.code is None:
                    raise  # bubble up to anmmrp.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~anmmrp.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "anmMRP Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in anmmrp.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + anmmrp.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = anmmrp.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if anmmrp.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
