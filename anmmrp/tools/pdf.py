...
from anmmrp.tools.misc import file_open
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with anmMRP's logo (optionally).
    ...
    anmmrp_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    anmmrp_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(anmmrp_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class AnmMrpPdfFileReader(PdfFileReader):
...
class AnmMrpPdfFileWriter(PdfFileWriter):
    ...
    def embed_anmmrp_attachment(self, attachment, subtype=None):
        assert attachment, "embed_anmmrp_attachment cannot be called without attachment."
