...
import anmmrp
from anmmrp.exceptions import UserError, ValidationError
from anmmrp.http import request
from anmmrp.models import check_method_name
from anmmrp.tools import DotDict
from anmmrp.tools.translate import _, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = anmmrp.registry(db).check_signaling()
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    result = retrying(partial(anmmrp.api.call_kw, recs, method, args, kw), env)
...
def execute(db, uid, obj, method, *args, **kw):
    with anmmrp.registry(db).cursor() as cr:
...
def retrying(func, env):
    ...
    :param anmmrp.api.Environment env: The environment where the registry
