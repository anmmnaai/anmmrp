...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.anmmrprc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store anmMRP data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'anmmrp_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "anmmrp.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="anmmrp.http:DEBUG", help='shortcut for --log-handler=anmmrp.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="anmmrp.sql_db:DEBUG", help='shortcut for --log-handler=anmmrp.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate anmMRP to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes anmmrp.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            anmmrp.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        anmmrp.netsvc.init_logger()
        self._warn_deprecated_options()
        anmmrp.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'anmmrp.conf')
        else:
            rcfilepath = os.path.expanduser('~/.anmmrprc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.anmmrprc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.anmmrprc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('ANMMRP_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from anmmrp.modules.module import MANIFEST_NAMES
