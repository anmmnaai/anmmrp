...
from anmmrp import SUPERUSER_ID, api
from anmmrp.exceptions import ValidationError
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=anmmrp.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
                  relativedelta=relativedelta,
                  version=anmmrp.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('anmmrp.tools.convert.init').error(
        ...
        # invoke method
        return anmmrp.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                ...
                groups.append(anmmrp.Command.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(anmmrp.Command.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec, extra_vals=None):
        ...
        for field in rec.findall('./field'):
            ...
            if f_search:
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [anmmrp.Command.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler anmmrp.tools.convert:DEBUG" for complete traceback
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <anmmrp> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['anmmrp', 'data', 'openerp']
