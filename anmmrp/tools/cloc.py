...
import anmmrp
from anmmrp.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in anmmrp.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [anmmrp.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(anmmrp.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be anmmrp/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = anmmrp.registry(config['db_name'])
        with registry.cursor() as cr:
            uid = anmmrp.SUPERUSER_ID
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="anmMRP cloc", lines="Line", other="Other", code="Code")
        s += hr
