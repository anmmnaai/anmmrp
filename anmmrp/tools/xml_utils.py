...
class anmmrp_resolver(etree.Resolver):
    """anmMRP specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None, prefix=None):
    ...
    :param anmmrp.api.Environment env: If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(anmmrp_resolver(env, prefix))
...
def load_xsd_files_from_url(env, url, file_name=None, force_reload=False,
    ...
    :param anmmrp.api.Environment env: environment of calling module
    ...
    :rtype: anmmrp.api.ir.attachment | bool
...
def validate_xml_from_attachment(env, xml_content, xsd_name, reload_files_function=None, prefix=None):
    ...
    :return: the result of the function :func:`anmmrp.tools.xml_utils._check_with_xsd`
    """
