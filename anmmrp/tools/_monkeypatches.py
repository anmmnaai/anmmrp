...
def literal_eval(expr):
    ...
    # can be overridden by setting the ANMMRP_LIMIT_LITEVAL_BUFFER buffer_size_environment variable

    buffer_size = 102400
    buffer_size_env = os.getenv("ANMMRP_LIMIT_LITEVAL_BUFFER")

    if buffer_size_env:
        if buffer_size_env.isdigit():
            buffer_size = int(buffer_size_env)
        else:
            _logger.error("ANMMRP_LIMIT_LITEVAL_BUFFER has to be an integer, defaulting to 100KiB")
