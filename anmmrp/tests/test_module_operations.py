...
import anmmrp
from anmmrp.tools import config, topological_sort, unique
from anmmrp.netsvc import init_logger
from anmmrp.tests import standalone_tests
import anmmrp.tests.loader

_logger = logging.getLogger('anmmrp.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with anmmrp.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of anmMRP modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra anmMRP modules")
a
...
def test_cycle(args):
    ...
    with anmmrp.registry(args.database).cursor() as cr:
        env = anmmrp.api.Environment(cr, anmmrp.SUPERUSER_ID, {})
...
def test_standalone(args):
    ...
    registry = anmmrp.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        anmmrp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        anmmrp.tools.config['addons_path'] = ','.join([args.addons_path, anmmrp.tools.config['addons_path']])
        if args.data_dir:
            anmmrp.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'anmmrp.modules.loading': {'level': 'CRITICAL'},
            'anmmrp.sql_db': {'level': 'CRITICAL'},
            'anmmrp.models.unlink': {'level': 'WARNING'},
            'anmmrp.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
