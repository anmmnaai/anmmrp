...
__author__ = anmmrp.release.author
__version__ = anmmrp.release.version

# Also use the `anmmrp` logger for the main script.
_logger = logging.getLogger('anmmrp')

re._MAXCACHE = 4096  # default is 512, a little too small for anmmrp
...
def report_configuration():
    ...
    """
    config = anmmrp.tools.config
    _logger.info("anmMRP version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', anmmrp.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = anmmrp.tools.config
    if not anmmrp.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = anmmrp.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            anmmrp.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = anmmrp.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = anmmrp.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    anmmrp.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                anmmrp.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except anmmrp.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        anmmrp.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = anmmrp.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the anmmrp server (default command)"""
    def run(self, args):
        anmmrp.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
