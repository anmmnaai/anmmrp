...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('anmmrp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    if sys.version_info[:2] == (3, 9):
        ...
        warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='anmmrp')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='anmmrp.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("ANMMRP_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'anmmrp.http.rpc.request:INFO',
    'anmmrp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['anmmrp:DEBUG', 'anmmrp.sql_db:INFO', 'anmmrp.http.rpc:DEBUG'],
    'debug_rpc': ['anmmrp:DEBUG', 'anmmrp.sql_db:INFO', 'anmmrp.http.rpc.request:DEBUG'],
    'debug': ['anmmrp:DEBUG', 'anmmrp.sql_db:INFO'],
    'debug_sql': ['anmmrp.sql_db:DEBUG'],
    'info': [],
    'runbot': ['anmmrp:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['anmmrp:WARNING', 'werkzeug:WARNING'],
    'error': ['anmmrp:ERROR', 'werkzeug:ERROR'],
    'critical': ['anmmrp:CRITICAL', 'werkzeug:CRITICAL'],
}
