...
import anmmrp
from anmmrp import SUPERUSER_ID
from anmmrp.exceptions import AccessDenied
import anmmrp.release
import anmmrp.sql_db
import anmmrp.tools
from anmmrp.sql_db import db_connect
from anmmrp.release import version_info
from anmmrp.tools import find_pg_tool, exec_pg_environ
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not anmmrp.tools.config['list_db']:
            _loggeanmmrpr.error('Database management functions blocked, admin disabled database listing')
            raise AccessDenied()
...
def check_super(passwd):
    if passwd and anmmrp.tools.config.verify_admin_password(passwd):
        return True
    raise anmmrp.exceptions.AccessDenied()

# This should be moved to anmmrp.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = anmmrp.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            anmmrp.modules.db.initialize(cr)
            anmmrp.tools.config['load_language'] = lang
            cr.commit()

        registry = anmmrp.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = anmmrp.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = anmmrp.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = anmmrp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = anmmrp.tools.config['db_template']
    ...
    try:
        db = anmmrp.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if anmmrp.tools.config['unaccent']:
                ...
                # But in the case of anmMRP, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    anmmrp.sql_db.close_db(db_original_name)
    db = anmmrp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = anmmrp.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = anmmrp.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            anmmrp.modules.neutralize.neutralize_database(cr)

    from_fs = anmmrp.tools.config.filestore(db_original_name)
    to_fs = anmmrp.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    anmmrp.modules.registry.Registry.delete(db_name)
    anmmrp.sql_db.close_db(db_name)
    ...
    fs = anmmrp.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'anmmrp_dump': '1',
        'db_name': cr.dbname,
        'version': anmmrp.release.version,
        'version_info': anmmrp.release.version_info,
        'major_version': anmmrp.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = anmmrp.tools.config.filestore(db_name)
            ...
            if stream:
                anmmrp.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                anmmrp.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = anmmrp.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = anmmrp.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    anmmrp.modules.registry.Registry.delete(old_name)
    anmmrp.sql_db.close_db(old_name)
    ...
    old_fs = anmmrp.tools.config.filestore(old_name)
    new_fs = anmmrp.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    anmmrp.tools.config.set_admin_password(new_password)
    anmmrp.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        anmmrp.tools.config['update']['base'] = True
        anmmrp.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@anmmrp.tools.mute_logger('anmmrp.sql_db')
...
def list_dbs(force=False):
    if not anmmrp.tools.config['list_db'] and not force:
        raise anmmrp.exceptions.AccessDenied()

    if not anmmrp.tools.config['dbfilter'] and anmmrp.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, anmMRP will not
        ...
        res = sorted(db.strip() for db in anmmrp.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [anmmrp.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of anmMRP
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if anmmrp.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        anmmrp.sql_db.close_db(database_name)
...
def exp_list_lang():
    return anmmrp.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(anmmrp.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
