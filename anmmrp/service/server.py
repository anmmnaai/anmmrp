...
import anmmrp
from anmmrp.modules import get_modules
from anmmrp.modules.registry import Registry
from anmmrp.release import nt_service_name
from anmmrp.tools import config
from anmmrp.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'anmmrp.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The ANMMRP_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("ANMMRP_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by anmMRP.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(anmmrp, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in anmmrp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="anmmrp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from anmmrp.addons.base.models.ir_cron import ir_cron
        conn = anmmrp.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = anmmrp.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(anmmrp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="anmmrp.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="anmmrp.service.httpd")
        t.daemon = True
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(anmmrp, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = anmmrp.tests.result._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('anmmrp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(anmmrp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The ANMMRP_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("ANMMRP_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = anmmrp.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from anmmrp.addons import base
    ...
    def start(self):
        ...
        dbconn = anmmrp.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(anmmrp.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            anmmrp.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if anmmrp.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from anmmrp.tests.suite import AnmMrpSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = anmmrp.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             anmmrp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the anmmrp http server and cron processor.
    """
    global server
    ...
    if anmmrp.evented:
        server = GeventServer(anmmrp.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(anmmrp.http.root)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which anmMRP is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(anmmrp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not anmmrp.evented:
        if inotify:
    ...
    if getattr(anmmrp, 'phoenix', False):
        _reexec()
