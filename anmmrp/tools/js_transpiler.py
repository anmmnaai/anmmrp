"""
This code is what let us use ES6-style modules in anmmrp.
Classic anmMRP modules are composed of a top-level :samp:`anmmrp.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@anmmrp-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** anmmrp-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual anmmrp one with minimal changes.
...
from anmmrp.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom anmmrp modules.
    ...
    legacy_anmmrp_define = get_aliased_anmmrp_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_anmmrp_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_anmmrp_define:
        content += legacy_anmmrp_define
    return content
...
def url_to_module_path(url):
    """
    anmMRP modules each have a name. (anmmrp.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_anmmrp_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the anmmrp.define call.
    ...
    """
    return f"""anmmrp.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ANMMRP_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @anmmrp-module                              # @anmmrp-module
...
def is_anmmrp_module(content):
    """
    Detect if the file is a native anmmrp module.
    We look for a comment containing @anmmrp-module.

    :param content: source code
    :return: is this a anmmrp module that need transpilation ?
    """
    result = ANMMRP_ODOO_MODULE_RE.match(content)
    return bool(result)


def get_aliased_anmmrp_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the anmmrp source base being:
    ...
    > anmmrp.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @anmmrp-module */
        // after
        /** @anmmrp-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @anmmrp-module */
        // after
        /** @anmmrp-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = ANMMRP_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nanmmrp.define(`%s`, ['%s'], function (require) {
                        return require('%s');
