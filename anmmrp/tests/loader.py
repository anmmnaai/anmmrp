...
from .suite import AnmMrpSuite
from .result import AnmMrpTestResult


def get_test_modules(module):
    """ Return a list of module for the addons potentially containing tests to
    feed unittest.TestLoader.loadTestsFromModule() """
    results = _get_tests_modules(importlib.util.find_spec(f'anmmrp.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"anmmrp.upgrade.{module}",
        f"anmmrp.addons.{module}.migrations",
        f"anmmrp.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return AnmMrpSuite(sorted(tests, key=lambda t: t.test_sequence))
