...
import anmmrp
from anmmrp.http import Controller, route, dispatch_rpc, request, Response
from anmmrp.fields import Date, Datetime, Command
from anmmrp.tools import lazy, ustr
from anmmrp.tools.misc import frozendict
...
# User code must use the exceptions defined in ``anmmrp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, anmmrp.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, anmmrp.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, anmmrp.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, anmmrp.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, anmmrp.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(anmmrp.tools.exception_to_unicode(e), formatted_info)

    return xmlrpc.client.dumps(fault, allow_none=None, encoding=None)


class AnmMrpMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, anmmrp is sending binary as base64 string.
...
xmlrpc.client.Marshaller = AnmMrpMarshaller
