# -*- coding: utf-8 -*-
# Part of amnMRP. See LICENSE file for full copyright and licensing details.
...
from anmmrp.exceptions import CacheMiss
from anmmrp.osv import expression
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~anmmrp.models.Model.read_group`
    ...
    def setup(self, model):
        ...
        if not self._setup_done:
            ...
            if self.states:
                warnings.warn(f'Since anmMRP 17, property {self}.states is no longer supported.')
    ...
    def groupable(self):
        """
        Return whether the field may be used for grouping in :meth:`~anmmrp.models.BaseModel.read_group`.
        """
    ...
    def mapped(self, records):
        ...
        over a simple call to `~anmmrp.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~anmmrp.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~anmmrp.fields.Float.round()` to round a float with the given precision.
    :func:`~anmmrp.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~anmmrp.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field):
    ...
    :class:`res_currency<anmmrp.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <anmmrp.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-anmmrp: currency may not be already initialized if it is
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`anmmrp.tools.image.ImageProcess` for maximum image resolution (default: ``50e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~anmmrp.models.Model._check_company`. Add a default company
...
class Properties(Field):
    ...
    like anmMRP fields but without being independently stored in database.
    ...
    current record. The container must have a :class:`~anmmrp.fields.PropertiesDefinition`
...
class PropertiesDefinition(Field):
    """ Field used to define the properties definition (see :class:`~anmmrp.fields.Properties`
...
class Command(enum.IntEnum):
    """
    :class:`~anmmrp.fields.One2many` and :class:`~anmmrp.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~anmmrp.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~anmmrp.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~anmmrp.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~anmmrp.fields.One2many` relation, the given record
