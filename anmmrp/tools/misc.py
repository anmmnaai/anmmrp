...
import anmmrp
import anmmrp.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from anmmrp.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if anmmrp.tools.config['db_host']:
        env['PGHOST'] = anmmrp.tools.config['db_host']
    if anmmrp.tools.config['db_port']:
        env['PGPORT'] = str(anmmrp.tools.config['db_port'])
    if anmmrp.tools.config['db_user']:
        env['PGUSER'] = anmmrp.tools.config['db_user']
    if anmmrp.tools.config['db_password']:
        env['PGPASSWORD'] = anmmrp.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',), env=None):
    ...
    addons_paths = anmmrp.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None, env=None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/anmmrp/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with anmmrp.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with anmmrp.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('anmmrp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('anmmrp.foo.bar'):
            do_suff()
    """
...
def ignore(*exc):
    warnings.warn("Since 16.0 `anmmrp.tools.ignore` is replaced by `contextlib.suppress`.", DeprecationWarning, stacklevel=2)
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = anmmrp.fields.Datetime.from_string(value)
            value = anmmrp.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = anmmrp.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = anmmrp.fields.Datetime.from_string(value)
    else:
        timestamp = value
