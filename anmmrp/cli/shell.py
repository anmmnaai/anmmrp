...
class Shell(Command):
    """Start anmmrp in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        config.parse_config(args)
        anmmrp.cli.server.report_configuration()
        anmmrp.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = anmmrp.api.Environment(cr, uid, {})['res.users'].context_get()
                env = anmmrp.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
