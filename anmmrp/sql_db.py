...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # anmMRP Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri):
    ...
    :rtype: (str, dict)
    """
    if 'ANMMRP_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['ANMMPR_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
        app_name = "anmmrp-%d" % os.getpid()
...
def db_connect(to, allow_uri=False):
    ...
    if _Pool is None:
        _Pool = ConnectionPool(int(
            anmmrp.evented and tools.config['db_maxconn_gevent']
...
def close_db(db_name):
    """ You might want to call anmmrp.modules.registry.Registry.delete(db_name) along this function."""
    global _Pool
    if _Pool:
