...
import anmmrp
import anmmrp.modules.db
import anmmrp.modules.graph
import anmmrp.modules.migration
import anmmrp.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('anmmrp.tests')
...
def force_demo(env):
    ...
    """
    graph = anmmrp.modules.graph.Graph()
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = anmmrp.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = anmmrp.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = anmmrp.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['anmmrp.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = anmmrp.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            loader = anmmrp.tests.loader
            suite = loader.make_suite([module_name], 'at_install')
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), anmmrp.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = anmmrp.sql_db.sql_counter - tests_q0
        ...
        extra_queries = anmmrp.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   anmmrp.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not anmmrp.modules.db.is_initialized(cr):
        ...
        if update_module and anmmrp.tools.table_exists(cr, 'ir_model_fields'):
        ...
        migrations = anmmrp.modules.migration.MigrationManager(cr, graph)
        for package in graph:
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                registry = anmmrp.modules.registry.Registry.new(
