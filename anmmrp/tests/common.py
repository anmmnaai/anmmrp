# -*- coding: utf-8 -*-
"""
The module :mod:`anmmrp.tests.common` provides unittest test cases and a few
...
import anmmrp
from anmmrp import api
from anmmrp.models import BaseModel
from anmmrp.exceptions import AccessError
from anmmrp.modules.registry import Registry
from anmmrp.service import security
from anmmrp.sql_db import BaseCursor, Cursor
from anmmrp.tools import float_compare, single_email_re, profiler, lower_logging, SQL
from anmmrp.tools.misc import find_in_path, mute_logger
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 17.0: anmmrp.tests.common.Form is deprecated, use anmmrp.tests.Form",
...
# The anmmrp library is supposed already configured.
ADDONS_PATH = anmmrp.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = anmmrp.SUPERUSER_ID
...
def get_db_name():
    db = anmmrp.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding anmMRP module name.
    """
    def register(func):
        # register func by anmmrp module name
        if func.__module__.startswith('anmmrp.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most anmmrp operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('anmmrp.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for anmMRP-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.anmmrp.com/r/Vmh
    ...
    def run(self, result):
        ...
        if getattr(testMethod, '_retry', True) and getattr(self, '_retry', True):
            tests_run_count = int(os.environ.get('ANMMRP_TEST_FAILURE_RETRIES', 0)) + 1
        else:
            tests_run_count = 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~anmmrp.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        with patch('anmmrp.models.BaseModel.user_has_groups', user_has_groups):
            yield
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('anmmrp.sql_db.Cursor.execute', execute):
            with patch('anmmrp.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/anmmrp/addons/" in filename:
                        filename = filename.rsplit("/anmmrp/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(anmmrp.modules.registry._logger.name)
    ...
    def _gc_filestore(cls):
        ...
        with anmmrp.registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, anmmrp.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.registry = anmmrp.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, anmmrp.SUPERUSER_ID, {})
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot'):
    ...
    screenshots_dir = pathlib.Path(anmmrp.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_class, headless=True):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_anmmrp')

        otc = anmmrp.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way anmMRP widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{anmmrp.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('anmmrp.service.http.request.')]
        ...
        if request_threads:
            ...
            anmmrp.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        anmmrp.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            anmmrp.http.root.session_store.delete(self.session)

        self.session = session = anmmrp.http.root.session_store.new()
        session.update(anmmrp.http.get_default_session(), db=get_db_name())
        session.context['lang'] = anmmrp.http.DEFAULT_LANG
        ...
        anmmrp.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return f"http://{HOST}:{anmmrp.tools.config['http_port']}"
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "anmmrp.startTour('%s', %s)" % (tour_name, json.dumps(options)))
        ready = kwargs.pop('ready', "anmmrp.isTourReady('%s')" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('anmmrp.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from anmmrp.tests.common have a test_tags
