...
import anmmrp
import anmmrp.tools as tools
import anmmrp.release as release
from anmmrp.tools import pycompat
from anmmrp.tools.misc import file_path
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `anmmrp.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^anmmrp\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("anmmrp.addons.base.maintenance.migrations", "anmmrp.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``anmmrp.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook anmmrp.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in anmmrp.addons.__path__:
        anmmrp.addons.__path__.append(dd)

    # hook anmmrp.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in anmmrp.addons.__path__:
            anmmrp.addons.__path__.append(ad)

    # hook anmmrp.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in anmmrp.addons.__path__ and os.path.isdir(base_path):
        anmmrp.addons.__path__.append(base_path)

    # hook anmmrp.upgrade on upgrade-path
    from anmmrp import upgrade
    ...
    # create decrecated module alias from anmmrp.addons.base.maintenance.migrations to anmmrp.upgrade
    spec = importlib.machinery.ModuleSpec("anmmrp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["anmmrp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["anmmrp.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to anmmrp and "crm"-like to anmmrp.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in anmmrp.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = anmmrp.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(anmmrp.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'anmmrp.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in anmmrp.addons.__path__:
